...
app.post('/api/analyzeText', async (req, res) => {
    ...
    try {
        const wordCounts = {}; // { "単語(基本形)": { count: 0, pos: "品詞カテゴリ" } }
        let processedTokensCount = 0; // 処理したトークン数をカウント（デバッグ用）

        for (const text of textList) {
            if (!text || typeof text !== 'string') continue;
            const tokens = tokenizer.tokenize(text);
            processedTokensCount += tokens.length; // トークン数を加算

            tokens.forEach(token => {
                const pos = token.pos;                 // 品詞 (例: 名詞, 動詞, ...)
                const posDetail1 = token.pos_detail_1; // 品詞細分類1 (例: 一般, サ変接続, 自立, 非自立, ...)
                const posDetail2 = token.pos_detail_2; // 品詞細分類2
                const posDetail3 = token.pos_detail_3; // 品詞細分類3
                // 基本形があればそれを、なければ表層形を使う
                const wordBase = token.basic_form !== '*' ? token.basic_form : token.surface_form;

                let targetPosCategory = null; // '名詞', '動詞', '形容詞', '感動詞' のいずれか

                // --- 品詞フィルタリング条件を調整 ---
                if (pos === '名詞') {
                    // 一般名詞、固有名詞、サ変接続名詞を拾う
                    if (['一般', '固有名詞', 'サ変接続', '形容動詞語幹'].includes(posDetail1)) {
                       targetPosCategory = '名詞';
                    }
                    // 接尾辞の一部 (例: '〜感', '〜性' など) も名詞として含める場合 (必要なら)
                    // else if (posDetail1 === '接尾' && ['一般', '人名', '地域', 'サ変接続'].includes(posDetail2)) {
                    //    targetPosCategory = '名詞';
                    // }
                } else if (pos === '動詞') {
                    // 自立動詞（基本形）を拾う (サ変接続も含む)
                    if (posDetail1 === '自立' || posDetail1 === 'サ変接続') { // ★ サ変接続も動詞として扱う
                       targetPosCategory = '動詞';
                    }
                     // 非自立動詞 (例: '〜ている' の 'いる') は基本除外するが、必要なら含める
                    // else if (posDetail1 === '非自立') {
                    //    targetPosCategory = '動詞';
                    // }
                } else if (pos === '形容詞') {
                    // 自立形容詞（基本形）を拾う
                    if (posDetail1 === '自立') {
                       targetPosCategory = '形容詞';
                    }
                     // 非自立形容詞 (例: '〜ない', '〜やすい') は基本除外
                    // else if (posDetail1 === '非自立') {
                    //    targetPosCategory = '形容詞';
                    // }
                } else if (pos === '感動詞') {
                    targetPosCategory = '感動詞';
                }
                // --- フィルタリング条件ここまで ---

                // 対象品詞で、かつ1文字より長い場合、かつ数字でない場合
                if (targetPosCategory && wordBase.length > 1 && !/^[0-9]+$/.test(wordBase)) {
        ...
        console.log(`Processed ${processedTokensCount} tokens in total.`); // デバッグログ追加

        const analysisResult = Object.entries(wordCounts).map(([word, data]) => ({
            word: word,
            score: data.count, // score = count
            pos: data.pos
        }));
        ...
        // console.log("Top 10 words:", analysisResult.slice(0, 10)); // 上位10件をログ出力（デバッグ用）
