...
// const OpenAI = require('openai'); // kuromoji を使うので OpenAI は不要
const kuromoji = require('kuromoji'); // kuromoji ライブラリを追加
...
// const OPENAI_API_KEY = process.env.OPENAI_API_KEY; // 不要
// ------------------------------
    ...
    try {
        ...
        for (const text of textList) {
            ...
            tokens.forEach(token => {
                ...
                if (pos === '名詞' && posDetail1 !== '数' && posDetail1 !== '代名詞' && posDetail1 !== '非自立' && posDetail1 !== '接尾') { targetPosCategory = '名詞'; }
                else if (pos === '動詞' && posDetail1 === '自立') { targetPosCategory = '動詞'; }
                else if (pos === '形容詞' && posDetail1 === '自立') { targetPosCategory = '形容詞'; }
                else if (pos === '感動詞') { targetPosCategory = '感動詞'; }

                if (targetPosCategory && wordBase.length > 1) { // 1文字より長い単語のみ
                    if (!wordCounts[wordBase]) { wordCounts[wordBase] = { count: 0, pos: targetPosCategory }; }
                    wordCounts[wordBase].count++;
                }
            });
        }
        const analysisResult = Object.entries(wordCounts).map(([word, data]) => ({ word: word, score: data.count, pos: data.pos }));
        analysisResult.sort((a, b) => b.score - a.score); // 頻度で降順ソート
        console.log(`kuromoji analysis complete. Found ${analysisResult.length} words.`);
        res.json({ totalDocs: textList.length, results: analysisResult });
