...
- 各summary, suggestion, overall.summaryは指定された文字数を目安に記述してください。

# 重要：記述スタイルとトーンの指定
1. **名称の統一**:
   - 文中で対象の病院に言及する際は、具体的な名称（${clinicName}）は使用せず、**必ず「貴院」という言葉に置き換えて**ください。

2. **トーン（上から目線を避ける）**:
   - 「〜すべきである」「〜だ」「〜が必要だ」といった断定的な言い切りや、指示的な強い表現は避けてください。
   - 相手に寄り添う丁寧な姿勢を示し、「〜が見込まれます」「〜と考えられます」「〜の可能性があります」「〜はいかがでしょうか」といった、**推察や提案を含んだ柔らかい文末表現**を徹底してください。

3. **出力制限**:
   - 出力は必ず指定されたJSONフォーマットに従い、それ以外のテキスト（挨拶や解説など）は一切含めないでください。`;
